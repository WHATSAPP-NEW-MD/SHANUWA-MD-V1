...
async (conn, mek, m, { from, q, reply, isOwner }) => {
    ...
    if (key === 'AUTO_RECORDING' && !['true', 'false'].includes(newValue)) {
        return reply("😓 *Invalid value for AUTO_RECODING. Please use `true` or `false`.*");
    }
    ...
    try {
        ...
        reply(`*You Set Up Update Successful ✘.*\n\n*✅ This is already set!* *${key}* ➠ ${newValue} ${finalMode ? `\n*Mode:* ${finalMode}` : ''}`);
