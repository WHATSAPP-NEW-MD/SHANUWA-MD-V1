...
if (!fs.existsSync(__dirname + '/auth_info_baileys/creds.json')) {
    if (config.SESSION_ID) {
      const sessdata = config.SESSION_ID.replace("SHANUWA-MD=","")
...
const prefix = config.PREFIX
    ...
    conn.ev.on('connection.update', async (update) => {
        ...
        if (connection === 'close') {
            ...
            let up = `🚀SHANUWA MD CONNECTED SUCCESSFUL✅ 

> THANK FOR YOU USING MY BOT  *👨‍💻SHANUWA MD*
...
conn.sendMessage(ownerNumber + "@s.whatsapp.net", { image: { url: `https://i.ibb.co/bMfXXPrd/Tharusha-Md.jpg` }, caption: up })
}
})
    ...
    conn.ev.on('messages.upsert', async (mek) => {
        try {
            ...
            const pushname = mek.pushName || 'Sinbore'
            const developers = '94778619990'
...
app.get("/", (req, res) => {
res.send("📟 SHANUWA MD Working successfully!");
});
